    ...
    $.scrollorama = function(options) {
		...
		function getCurrBlockIndex(scrollTop) {
			...
			for (i=0; i<blocks.length; i++) {
				...
				if (blocks[i].top <= scrollTop +scrollorama.settings.offset+1/*genii*/ /*scrollorama.settings.offset*/) { currBlockIndex = i; }
			}
			if(scrollTop < blocks[0].top-scrollorama.settings.offset-1 || scrollTop <= 0) {currBlockIndex = -1;}
