...
var isotope_start_height;
(function($) {
    $.scrolldeck = function(options) {
			isotope_widget = $('.isotope_widget').height();
		...
		function updateNav() {
			if (buttons) {
				buttons.parent().removeClass('current');
				var currSection = -1;
				...
				if (currSection != -1) {
					//buttons.eq(currSection).addClass('current');
						buttons.eq(currSection).parent().addClass('current');
				}
			}
		}
		...
		function scrollToSlide(slide) {
			...
			$(window).scrollTo(slide, {
				...
				offset: -scrolldeck.settings.offset/*genii*/
			});
		}
